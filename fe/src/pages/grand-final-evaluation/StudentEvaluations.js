...
export default function GFStudentEvaluations({
  evaluations,
  setEvaluations,
  milestone,
  filterForm,
  ...
  const handleProcessRowUpdate = (newRow, oldRow) => {
    ...
    if (teamId !== null && isChangeGrade) {
      nRows.forEach((row, index) => {
        if (
          row.teamName === newRow.teamName &&
          isNullOrEmpty(row[gradeIdx]) &&
          !isNullOrEmpty(row.email) &&
          !isNullOrEmpty(newRow[gradeIdx])
        ) {
          nRows[index] = {
            ...row,

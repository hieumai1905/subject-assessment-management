...
import { teacherRoles } from "../../data/ConstantData";
              ...
              {modalType == "add" && (
                <Col md="6">
                  <div className="form-group">
                    ...
                    <RSelect
                      {...register("roleId", { required: "Trường này là bắt buộc" })}
                      options={teacherRoles}
                      value={teacherRoles.find((s) => s.value === formData?.roleId)}
                      onChange={(e) => setFormData({ ...formData, roleId: e.value })}
                    />
                    {errors.roleId && <span className="invalid text-danger">{errors.roleId.message}</span>}
                  </div>
                </Col>
              )}
              {modalType == "edit" && (

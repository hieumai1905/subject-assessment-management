...
export default function ImportStudentEvalModal({ modal, setModal, rows, setRows, evaluations, teams }) {
  const {
    reset,
    register,
    handleSubmit,
  ...
  const onSubmit = async () => {
    ...
    if (!isExistField(formData[0], mileTitle)) {
      toast.error(`File nhập không đúng định dạng`, {
        position: toast.POSITION.TOP_CENTER,
      });
      return false;
    }
    ...
    try {
      const response = await authApi.post("/evaluation/evaluate-student", changed);
      ...
      toast.error("Xảy ra lỗi khi nhập đánh giá", {
        position: toast.POSITION.TOP_CENTER,
      });
    } finally {
      setIsFetching(false);
